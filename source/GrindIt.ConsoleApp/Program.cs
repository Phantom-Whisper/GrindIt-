...
namespace GrindIt.ConsoleApp
{
    internal static class Program
    {
        /// <summary>
        /// This is the main menu for the console app.
        /// </summary>
        /// <returns>An integer corresponding to the user choice.</returns>
        private static int MainMenu()
        {
            int choice;
            
            Console.WriteLine(@"
            ________  ________  ___  ________   ________  ___  _________  ___       
            |\   ____\|\   __  \|\  \|\   ___  \|\   ___ \|\  \|\___   ___\\  \      
            \ \  \___|\ \  \|\  \ \  \ \  \\ \  \ \  \_|\ \ \  \|___ \  \_\ \  \     
             \ \  \  __\ \   _  _\ \  \ \  \\ \  \ \  \ \\ \ \  \   \ \  \ \ \  \    
              \ \  \|\  \ \  \\  \\ \  \ \  \\ \  \ \  \_\\ \ \  \   \ \  \ \ \__\   
               \ \_______\ \__\\ _\\ \__\ \__\\ \__\ \_______\ \__\   \ \__\ \|__|   
                \|_______|\|__|\|__|\|__|\|__| \|__|\|_______|\|__|    \|__|     ___ 
                                                                                |\__\
                                                                                \|__|");
            Console.WriteLine("Welcome to GrindIt!\nThis is the main menu.");
            Console.WriteLine("'Nutrition' refers to all aspects of healthy living, while 'Workout' covers " +
                              "the sporting aspects.");
            Console.WriteLine("1. Nutrition ");
            Console.WriteLine("2. Workout");
            ...
            {
                Console.WriteLine("Invalid choice. Please choose between 1 or 2, or 9 to quit.");
                Console.Write("Enter your choice: ");
        ...
        /// <summary>
        /// This is the nutrition menu for the console app.
        /// </summary>
        /// <returns>An integer corresponding to the user choice.</returns>
        private static int NutritionMenu()
        {
            int choice;
            ...
            while (!int.TryParse(input, out choice) || (choice < 1 || (choice > 6 && choice != 9)))
            {
                Console.WriteLine("Invalid choice. Please choose a number between 1–6, or 9 to return.");
                Console.Write("Enter your choice: ");
                input = Console.ReadLine();
            }
        ...
        /// <summary>
        /// This is the workout menu for the console app.
        /// </summary>
        /// <returns>An integer corresponding to the user choice.</returns>
        private static int WorkoutMenu()
        {
            int choice;
        ...
        private static void Main()
        {
            ...
            List<Food> list =
             ...
             void RecordSleep()
             {
                 var bedTime = new DateTime(2025, 3, 1, 22, 35, 0);
                 var wakeUpTime = new DateTime(2025, 3, 2, 8, 5, 0);

                 var sleepSerializer = new SleepSerializer();

                 var newSleep = new Sleep(bedTime, wakeUpTime, sleepSerializer);
             ...
             void RecordMeal()
             {
                 ...
                 var changeInput = Console.ReadLine()?.Trim().ToLower();
             ...
             void ExercisesSave()
             {
                 ...
                 var exercisesList = serializer.Load<List<Exercise>>();
             ...
             void FoodSave()
             {
                 ...
                 var loadedFood = foodSerialize.Load<List<Food>>();

                 foreach (var food in loadedFood)
                 {
                     food.ShowFood();
                 }
             }
                 ...
                 while (!exit)
                 {
                     ...
                     switch (input)
                     {
                         ...
                         case "5":
                             ...
                             int.TryParse(input2, out var weight);
                             set.SetWeight(weight);
                             break;
                         case "6":
                             Console.WriteLine("Enter the reps: ");
                             var input3 = Console.ReadLine();
                             int.TryParse(input3, out var reps);
                             set.SetReps(reps);
                             break;
                         case "9":
                             exit = true;
             ...
             void CreateFood()
             {
                 ...
                 var serving = (int)GetValidIntInput("Enter the size of one serving: ");
                 var calories = GetValidIntInput($"Enter the amount of calories for {serving}: ");
                 var carbohydrates = GetValidIntInput($"Enter the amount of carbohydrates for {serving}: ");
                 var fat = GetValidIntInput($"Enter the amount of fat for {serving}: ");
                 var protein = GetValidIntInput($"Enter the amount of protein for {serving}: ");
                 var satFat = GetValidIntInput($"Enter the amount of saturated fat for  {serving}: ");
                 var transFat = GetValidIntInput($"Enter the amount of transFat for  {serving}:  ");
                 var cholesterol = GetValidIntInput($"Enter the amount of cholesterol for  {serving}: ");
                 var sodium = GetValidIntInput($"Enter the amount of sodium for {serving}: ");
                 var potassium = GetValidIntInput($"Enter the amount of potassium for {serving}: ");
                 var dietaryFiber = GetValidIntInput($"Enter the amount of dietary fiber for {serving}: ");
                 var sugar = GetValidIntInput($"Enter the amount of sugar for {serving}: ");
                 ...
                 Food food = new Food(name, calories, serving, carbohydrates, fat, protein, satFat, transFat, cholesterol, sodium, potassium, dietaryFiber, sugar, category);
             ...
             void CreateExercise()
             {
                 ...
                 var bodyW = input.Equals("Y", StringComparison.CurrentCultureIgnoreCase);

                 var exercise = new Exercise(name, bodyW);
                 Console.Clear();
                 ...
                 while (true)
                 {
                     ...
                     List<TargetedMuscles> selectedMuscles = [];
                     var allValid = true;

                     if (inputs != null)
                         foreach (var item in inputs)
                         {
                             if (int.TryParse(item.Trim(), out var res) && Enum.IsDefined(typeof(TargetedMuscles), res))
                             {
                                 var muscle = (TargetedMuscles)res;
             ...
             Meal CreateMeal()
             {
                 ...
                 while (true)
                 {
                     ...
                     if (int.TryParse(Console.ReadLine(), out var res) && Enum.IsDefined(typeof(MealType), res))
                     {
                 ...
                 while (true)
                 {
                     ...
                     if (int.TryParse(Console.ReadLine(), out var foodNumber) && foodNumber >= 0 && foodNumber <= list.Count)
                     {
                         if (foodNumber == 0) break;
                         Console.Write("Enter the serving portion of the food you to add (separate with ',': ");
                         float.TryParse(Console.ReadLine(), out var serving);
             ...
             void WaterManagement()
             {
                 ...
                 while (!exit)
                 {
                     ...
                     switch (choice)
                     {
                         case 1:
                             Console.Clear();
                             water.AddWater(cup.Size);
                             break;
                         case 2:
                             Console.Clear();
                             water.RemoveWater(cup.Size);
                             break;
                         case 3:
                             exit = true;
                             break;
             ...
             void DisplayCategoryEnum()
             {
                 ...
                 {
                     var categoryString = FoodCategoryHelper.ConvertToString(category);
             ...
             void DisplayTargetedMusclesEnum()
             {
                 ...
                 {
                     var musclesString = TargetedMusclesToString.ToString(targetedMuscles);
             ...
             void DisplayMealTypeEnum()
             {
                 ...
                 {
                     var mealString = MealTypeToString.ToString(meal);
